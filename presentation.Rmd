...
plot_5 <- ggplot(data=nyweather,
       mapping=aes(x=wind_vector_dir))+
  geom_histogram(
    binwidth=3
  )
plot_6 <- ggplot(data=nyweather,
...
- Dew Point v Scattered Cloud Percentage
...
---
class: center
As you can see this shows very little correlation so this relationship was not investigated further
```{r, echo=FALSE}
...
---
class: center
While this is not an exact linear model it is pretty close, enabling us to draw conclusions about the relationship between the chosen variables
```{r, echo=FALSE}
...
---
class: center
Despite using geom_jitter to split up the plots, the sheer amount of inputs shows some kind of relationship between both but not with out variations.
```{r, echo=FALSE}
...
---
#Citations
- https://www.ncei.noaa.gov/cdo-web/datasets#NORMAL_HLY 
- https://www.ncei.noaa.gov/pub/data/cdo/documentation/NORMAL_HLY_documentation.pdf 
- https://www.ncei.noaa.gov/metadata/geoportal/rest/metadata/item/gov.noaa.ncdc:C00824/html 
- https://www.google.com/url?sa=i&url=https%3A%2F%2Fpix4free.org%2Fphoto%2F365%2Fnew-york-city-night-cityscape.html&psig=AOvVaw2nNY_N4BvAAnPNvu7N7Pn5&ust=1732313064955000&source=images&cd=vfe&opi=89978449&ved=0CBQQjRxqFwoTCLjf5Jq37okDFQAAAAAdAAAAABAI  (Accessed on the 22/11/24)
